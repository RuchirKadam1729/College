...
int main(int argc, char const* argv[])
{
    ...
    struct job task1 = { .i = 1, .func = func };
    struct job task2 = { .i = 2, .func = func };
    struct job task3 = { .i = 3, .func = func };
    struct job task4 = { .i = 4, .func = func };
    struct job task5 = { .i = 5, .func = func };
    int ntask = 5;
    ready_queue = enqueue(ready_queue, task1);
    ready_queue = enqueue(ready_queue, task2);
    ready_queue = enqueue(ready_queue, task3);
    ready_queue = enqueue(ready_queue, task4);
    ready_queue = enqueue(ready_queue, task5);
