...
{
    sprintf(buf, "chars: %d, words: %d, sentences: %d", resultset.chars,resultset.words, resultset.sentences);
}
...
int main(void)
{
    ...
    while (1)
    {
        ...
        if (!fork())
        {
            ...
            int numbytes;
            while ((numbytes = recv(new_fd, client_msg, sizeof(client_msg), 0)) != 0)
            {
                if (numbytes == -1)
                {
                    perror("recv");
